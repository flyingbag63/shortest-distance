...
class GraphBuilder:
    ...
    def build(cls, rider: Rider, locations: List[Location]) -> Graph:
        ...
        distance_calculator: DistanceCalculator = DistanceCalculatorFactory.get_calculator(
            DistanceCalculationStrategy.HAVERSINE
        )
        nodes: List[NodeDTO] = node_transformer.transform_many(locations)
        rider_node: NodeDTO = next(
            filter(
                lambda node: node.is_rider(), nodes
            )
        )
        ...
        for from_node in nodes:
            for to_node in nodes:
                if from_node != to_node:
                    distance: Decimal = distance_calculator.calculate(from_node, to_node)
